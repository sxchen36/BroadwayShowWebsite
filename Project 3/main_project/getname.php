...
<?php
  ...
  $score = $_POST["show_score"];
  $price = $_POST["show_price"];
  $location = $_POST["show_location"];
  $date = $_POST["date"];
  $seat = $_POST["seat_location"];
 
  
  // create query 
  if ($score==0 && $price==0 && $location== "*" && $seat=="*"){
    $query = "CALL SearchType('$type');";
  } else if ($type=="*" && $price==0 && $location== "*" && $seat=="*"){
      $query = "CALL SearchScore($score);";
  } else if ($type=="*" && $score==0 && $location== "*" && $seat=="*"){
        $query = "CALL SearchPrice($price);";
  } else{
  $query = "CALL FindShow('$type',$score,$price,'$seat','2013-12-27');";
  }
